...
#
docker stack deploy -c docker-compose-oss.yaml elk
...
docker exec $kibana_container curl -s -o /dev/null --show-error -X POST 'http://localhost:5601/api/saved_objects/_import?overwrite=true' -H 'kbn-xsrf: true' --form file=@/tmp/saved_objects/kibana_dashboards_oss_7.10.2.ndjson
